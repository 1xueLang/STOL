...
class VGG(nn.Module):
    def __init__(
        ...
        self.avgpool = online.DoubleForward(nn.AdaptiveAvgPool2d((7, 7)))
        self.classifier = nn.Sequential(
            ...
            # neuron(**neuron_cfg),
            online.Dropout(p=dropout),
            online.DoubleForward(nn.Linear(512 * 7 * 7, num_classes)),
        )
        if init_weights:
...
def make_layers(
    ...
    for v in cfg:
        if v == "M":
            # n = layers[-1]
            # layers[-1] = online.DoubleForward(nn.MaxPool2d(kernel_size=2, stride=2))
            # layers.append(n)
            layers += [online.DoubleForward(nn.AvgPool2d(kernel_size=2, stride=2))]
        else:
            v = cast(int, v)
            conv2d = online.DoubleForward(nn.Conv2d(in_channels, v, kernel_size=3, padding=1))
            if batch_norm is not None:
                layers += [
                    conv2d, 
                    online.DoubleForward(batch_norm(v)),
                    neuron(**neuron_cfg),
                    # online.Dropout(p=dropout),
                ]
            else:
