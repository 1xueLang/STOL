...
import online
from ._shortcuts import shortcuts
...
def cifar10_feature(norm_layer, inplanes, neuron, n_config):
    nn_block = conv3x3_online(3, inplanes, stride=1, norm_layer=norm_layer)
    return nn.Sequential(nn_block, neuron(**n_config))

def cifar10dvs_feature(norm_layer, inplanes, neuron, n_config):
    nn_block = conv3x3_online(2, inplanes, stride=1, norm_layer=norm_layer)
    return nn.Sequential(nn_block, neuron(**n_config))

def classic_feature(norm_layer, inplanes, neuron, n_config):
    layers = [
        online.DoubleForward(nn.Conv2d(3, inplanes, kernel_size=7, stride=2, padding=3, bias=False)),
        online.DoubleForward(norm_layer(inplanes)),
        neuron(**n_config),
        online.DoubleForward(nn.MaxPool2d(kernel_size=3, stride=2, padding=1))
    ]
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.avgpool = online.DoubleForward(nn.AdaptiveAvgPool2d((1, 1)))
    ...
    def _make_layer(
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
                downsample = nn.Sequential(
                    conv1x1_online(self.inplanes, planes * block.expansion, stride=stride, norm_layer=norm_layer),
                    neuron(**n_config)
                )
