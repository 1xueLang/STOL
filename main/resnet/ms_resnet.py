...
from ._shortcuts import conv1x1, conv1x1_online
...
def cifar10_feature(type, norm_layer, inplanes, neuron, n_config):
    conv = nn.Conv2d(3, inplanes, kernel_size=3, stride=1, padding=1, bias=False)
    if type == 'A' or type == 'C':
        layers = nn.Sequential(
            online.DoubleForward(conv),
            online.DoubleForward(norm_layer(inplanes))
        )
    else:
        layers = online.DoubleForward(conv)
    
    return layers

def cifar10dvs_feature(type, norm_layer, inplanes, neuron, n_config):
    conv = nn.Conv2d(2, inplanes, kernel_size=3, stride=1, padding=1, bias=False)
    if type == 'A' or type == 'C':
        layers = [
            online.DoubleForward(conv),
            online.DoubleForward(norm_layer(inplanes))
...
def classic_feature(type, norm_layer, inplanes, neuron, n_config):
    layers = [
        online.DoubleForward(nn.Conv2d(3, inplanes, kernel_size=7, stride=2, padding=3)),
    ]
    if type == 'A' or type == 'C':
        layers.append(online.DoubleForward(norm_layer(inplanes))),
    if type == 'C':
        layers.append(neuron(**n_config))
        
    layers.append(online.DoubleForward(nn.MaxPool2d(kernel_size=3, stride=2, padding=1)))
...
class ResNet(nn.Module):
    def __init__(
        ...
        out_layers = []
        if self.type == 'A':
            out_layers.extend([neuron(**n_config),])
        elif self.type == 'B':
            out_layers.extend([
                online.DoubleForward(norm_layer(512 * block.expansion)), 
                neuron(**n_config), 
            ])
        out_layers.append(online.DoubleForward(nn.AdaptiveAvgPool2d((1, 1))))
        self.out_layer = nn.Sequential(*out_layers)
        self.fc = nn.Sequential(
            online.DoubleForward(online.Flattten(1)),
            online.DoubleForward(nn.Linear(512 * block.expansion, num_classes))
        )
        ...
        if zero_init_residual:
            for m in self.modules():
                if (isinstance(m, Bottleneck_A) or isinstance(m, Bottleneck_B) or isinstance(m, Bottleneck_C)) and m.bn3.weight is not None:
                    nn.init.constant_(m.bn3.weight, 0)  # type: ignore[arg-type]
                elif (isinstance(m, BasicBlock_A) or isinstance(m, BasicBlock_B) or isinstance(m, BasicBlock_C)) and m.bn2.weight is not None:
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            if self.type == 'A':
                downsample = nn.Sequential(
                    neuron(**n_config),
                    conv1x1_online(self.inplanes, planes * block.expansion, stride, norm_layer=norm_layer)
                )
            elif self.type == 'B':
                downsample = nn.Sequential(
                    online.DoubleForward(norm_layer(self.inplanes)),
                    neuron(**n_config),
                    online.DoubleForward(conv1x1(self.inplanes, planes * block.expansion, stride))
                )
            elif self.type == 'C':
                downsample = conv1x1_online(self.inplanes, planes * block.expansion, stride, norm_layer=norm_layer)
            else:
                raise ValueError('')
